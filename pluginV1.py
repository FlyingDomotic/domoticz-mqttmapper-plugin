...
class MqttClient:
    ...
    def onConnect(self, Connection, Status, Description):
        ...
        if (Status == 0):
            ID = F"Domoticz_{self.parameters['Key']}_{self.parameters['HardwareID']}_{int(time.time())}"
...
class pluginV1:
    ...
    def __init__(self, Parameters, Devices):
        Domoticz.Debug(F"pluginV1::__init__")
        # Domoticz tables
        self.parameters = Parameters
        self.devices = Devices
        # MQTT settings
        ...
        self.lastMqttCheckUtc = 0
        self.jsonData = None
        self.initDone = False
        self.switchTypes = DomoticzTypes()
    ...
    def getDevice(self, deviceName):
        for device in self.devices:
            if (self.devices[device].DeviceID == deviceName) :
                # Return device
    ...
    def dumpConfigToLog(self):
        for x in self.parameters:
            if self.parameters[x] != "":
                Domoticz.Log(F"'{x}': '{self.parameters[x]}'")
        Domoticz.Log(F"Device count: {str(len(self.devices))}")
        for x in self.devices:
            Domoticz.Log(F"Device: {x} - {str(self.devices[x])}")
    ...
    def onStart(self):
        ...
        self.mqttserveraddress = self.parameters["Address"].replace(" ", "")
        self.mqttserverport = self.parameters["Port"].replace(" ", "")

        # Load JSON mapping file
        jsonFile = self.parameters['HomeFolder'] + self.parameters["Mode1"]
        self.jsonData = None
        ...
        try:
            with open(parametersFile, "wt") as f:
                ...
                fields["mqttUsername"] = self.parameters["Username"]
                fields["mqttPassword"] = self.parameters["Password"]
